...
uint8_t LCD_TP_read_register(uint16_t reg, uint8_t size, uint8_t *data){
	int status = HAL_I2C_Mem_Read(&hi2c, LCD_TP_ADDRESS, reg, sizeof(reg), data, size,1000);
	return status;
}

uint8_t LCD_TP_write_register(uint16_t reg, uint8_t size, uint8_t *data){
	int status = HAL_I2C_Mem_Write(&hi2c, LCD_TP_ADDRESS, reg, sizeof(reg), data, size,5000);
	return status;
}
